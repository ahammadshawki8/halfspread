...
| **R11** | **No emoji and no long dashes anywhere in the website, the README, this file, code comments or commit messages.** No em dash, no en dash, no arrows-as-decoration, no status glyphs. Use plain words, a hyphen, or " - ". Applies to anything a judge or user can read, and to anything generated into the dashboard. Check with a sweep before committing. |
...
> [note] **`docs/paper/paper-annotated.md` in that repo is STALE** and still shows pre-correction numbers. Only trust `KNOWN-ISSUES.md`. The repo's Git LFS budget is exceeded, so data panels cannot be fetched.
...
- **Index options in paper:** SPX, SPXW, VIX, VIXW, DJX, XSP - European, cash-settled. [note] **Market data availability for these is UNVERIFIED** - Alpaca's launch post said index data was not yet in their Market Data offering. **Tier 0 must settle this.**
...
`SPY` [done] · `XSP` [done] · `SPXW` [done] · `SPX` returns empty (SPX is the AM-settled monthly root; **use SPXW** for weeklies/dailies).
 ...
 **We compute our own.** `pricing.py` must solve implied vol from mid price (Black-Scholes, bisection/Newton) and derive delta/gamma/theta/vega. This is an asset, not a setback: strike selection rests on a pricing layer we own and can show.
 ...
 **Relative execution cost explodes as you go OTM.** The textbook "sell the 15-20Δ strike" plan lands exactly where the spread eats 10-15% of mid *per leg* - roughly 20-30% of the credit gone at entry, before any risk is taken.

 **This is the project's core finding and it is our own measurement, not a citation.** There is a real, quantified tension: far-OTM = higher win rate but ruinous relative cost; near-ATM = cheap execution but lower win rate. Nobody in the field models it. **The agent's job is to find the strike that maximises net EV after measured entry cost - which will not be the strike a delta-only screen picks.**
...
[note] **The CLI fires immediately - there are no confirmation prompts.**

[note] **Git Bash mangles API paths.** `alpaca api POST /v2/orders` becomes
...
[note] **Console encoding.** The Windows console is cp1252; printing non-ASCII raises
...
| **Execution cost / slippage** | **~4** | [note] **Contested, not empty.** Lyceum ("execution-cost modeling"), Finly ("cost-modeled"), PrintRunner ("breaker at 2x costs"), "a continual learning agent" ("live execution quality") |
...
### Tier 0 - Foundations & the blocking check [done] COMPLETE
...
### Tier 1 - Read-only data layer [done] COMPLETE
...
### Tier 2 - Decision layer [done] COMPLETE
...
### Tier 3 - Execution [done] CORE COMPLETE
...
### Tier 3.5 - Orchestration [done] COMPLETE
...
- [done] ~~DEV keys~~ · [done] ~~COMP account at exactly $100,000~~ · [done] ~~index option data~~ - all resolved, see §5.1.
- [open] **Greeks must be computed in-house** (feed provides none). `pricing.py` is now on the critical path for Tier 1.
- [open] **Re-measure the cost curve during market hours.** §5.1 Finding 3 used 2026-09-02 closing quotes. Spreads widen intraday, especially after ~15:00 ET - confirm before the first COMP order.
- [open] Featherless AI ($25 free credits, no card) - optional partner integration; only if a bounded, honest role exists that doesn't contradict the deterministic-core design. Owner's call.
- [open] SPY assignment risk on an ITM finish (American, physically settled). Acceptable - only bites when the short strike is already breached - but `settle.py` must handle it.
