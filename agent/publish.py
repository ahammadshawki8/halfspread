...
def refresh_readme(payload: dict) -> bool:
    """Rewrite the measured block in the README from the journal.

    A hardcoded number in a README is stale the moment the agent trades again,
    and after the deadline nothing can be corrected by hand. So the figures are
    generated from the same journal the dashboard reads, carry the timestamp
    they were measured at, and name the command that reproduces them. A
    timestamped measurement does not go out of date: it is a fact about a
    ...
    f = (payload.get("verification") or {}).get("facts") or {}
    L = payload.get("ledger") or {}
    A = payload.get("account") or {}
    st = payload.get("spread_story") or {}
    ver = payload.get("verification") or {}
    emp = payload.get("empirical") or {}
    ...
    rows = [
        ("Cost to cross at the money, SPY",
         n(f.get("half_spread_atm_pct"), 2, suf="% of mid")),
        ("Cost to cross 2 to 3 points out",
         n(f.get("half_spread_far_pct"), 2, suf="% of mid")
         + (f"  ({f['cost_curve_ratio']}x worse)" if f.get("cost_curve_ratio") else "")),
        ("Same contract, open against latest",
         (f"{st.get('times_wider')}x more expensive to cross"
          + (f"  ({st.get('symbol')})" if st.get("symbol") else ""))
         if st.get("times_wider") else "not yet measured"),
        ("Largest decision-to-execution drift",
         n(f.get("max_abs_drift"), 2, pre="$", suf=" per contract")),
        ("Credit received", n(L.get("credit_received"), 2, pre="$")),
        ("Entry cost paid", n(L.get("entry_cost_paid"), 2, pre="$")),
        ("Exit cost paid", n(L.get("exit_cost_paid"), 2, pre="$")),
        ("Exit cost still at stake, open positions",
         n(L.get("exit_cost_at_stake"), 2, pre="$")),
        ("Account", n(A.get("pnl"), 2, pre="$")
         + f" against a {n(A.get('starting_equity'), 0, pre='$')} start"),
        ("Candidates priced / admitted",
         f"{(payload.get('scan_totals') or {}).get('candidates_priced', 0):,}"
         f" / {(payload.get('scan_totals') or {}).get('candidates_admissible', 0):,}"),
        ("Decisions not to trade", str(f.get("refusals", 0))),
        ("Sessions behind the breach distribution",
         f"{emp.get('sessions', 0):,} since {emp.get('start', '')}"),
        ("Claims reproducing from the journal",
         f"{ver.get('passed', 0)}/{ver.get('total', 0)}"),
    ]

    body = [
        BEGIN,
        f"<!-- generated by `python -m agent.publish`; do not edit by hand -->",
        "",
        f"**Measured at {when}.** Every figure below is generated from the",
    ...
    new = head + chr(10).join(body) + tail
    if new == text:
        return False
...
def main() -> int:
    ...
    print(f"  README measured block: {'updated' if refresh_readme(payload) else 'unchanged'}")
